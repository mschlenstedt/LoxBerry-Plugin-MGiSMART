...
#
# Refuses the installation when the system Python is too old for the gateway.
...
# first import - the dependencies themselves have no such requirement, so pip
# never notices. Better to say no here than to leave a broken plugin behind.
#
...
if ! command -v python3 >/dev/null 2>&1; then
	...
	echo "<FAIL> The MG iSMART plugin needs Python 3.12 or newer (Debian 13 'Trixie' or newer)."
	exit 2
fi
...
" 2>/dev/null
then
	echo "<OK> Python $HAVE satisfies the requirement (>= $MIN)."
	exit 0
fi

echo "<FAIL> This system has Python $HAVE, but saic-python-mqtt-gateway needs $MIN or newer."
echo "<FAIL> The gateway uses language features that do not exist before Python 3.12,"
echo "<FAIL> so it would install without error and then fail to start."
echo "<FAIL>"
echo "<FAIL> Debian 12 ('Bookworm') ships Python 3.11 and is not sufficient."
